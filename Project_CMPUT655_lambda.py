...
def td_error_return(data, weights, lambda_param=0.9, alpha_weights=0.1, gamma=0.99):
    """
    Compute TD errors and update weights using TD(λ).

    Parameters:
    - data (dict): Collected data with 'phi', 'r', 'a', 'done'.
    - weights (np.ndarray): Weight matrix for the state-action value function.
    - lambda_param (float): Trace decay rate (λ).
    - alpha_weights (float): Learning rate for weight updates.
    - gamma (float): Discount factor.

    Returns:
    - computed_rewards (list): List of cumulative rewards for each step.
    - weights (np.ndarray): Updated weights.
    - td_errors (list): List of TD errors for each time step.
    """
    phi_values = np.array(data["phi"])  # State features (T, features)
    rewards = np.array(data["r"])       # Rewards (T,)
    actions = np.array(data["a"])       # Actions (T,)
    done_flags = np.array(data["done"]) # Done flags (T,)
    T = len(rewards)  # Number of time steps

    td_errors = []
    computed_rewards = []
    z = np.zeros_like(weights)  # Eligibility traces (same shape as weights)

    for t in range(T):
        # Current state-action feature
        current_phi = phi_values[t].flatten()
        action_t = actions[t]

        # Compute value of the current state-action pair
        #V_s_t = np.dot(current_phi, weights[:, action_t])
        V_s_t = np.max(np.dot(current_phi, weights))

        # Compute the TD target
        r_t = rewards[t]
        if t + 1 < T and not done_flags[t]:  # Handle terminal state
            next_phi = phi_values[t + 1].flatten()
            next_action = actions[t + 1]
            #V_s_tp1 = np.dot(next_phi, weights[:, next_action])  # Value of next state-action pair
            V_s_tp1 = np.max(np.dot(next_phi, weights))
        else:
            V_s_tp1 = 0  # No future reward for terminal state
        ...
        z *= gamma * lambda_param  # Decay traces
        z[:, action_t] += current_phi  # Update trace for current action

        # Update weights
        weights += alpha_weights * td_error_t * z

        # Store cumulative reward (G)
...
def reinforce(method="none", lambda_param=0):
    """
    ...
    while tot_steps < max_steps:
        ...
        if method == "mc":
            ...
            returns, weights, td_errors = td_error_return(data, weights, lambda_param=0, alpha_weights=alpha_weights, gamma=gamma)
        elif method.startswith("lambda"):
            lambda_value = float(method.split()[1])  # Extract n_step value from method string
            returns, weights, td_errors = td_error_return(data, weights, lambda_param=lambda_value, alpha_weights=alpha_weights, gamma=gamma) 
        else:
        ...
        for t in range(T):
            ...
            if method == "mc":
                ...
                lambda_value = float(method.split()[1])  # Extract n-step value
                z_theta = gamma * lambda_value* z_theta + (gamma ** t)*grad_log_pi.reshape(theta.shape)
...
methods = ["mc", "td_zero", "lambda 0.2", "lambda 0.3", "lambda 0.4"]
...
for i, method in enumerate(methods):
    lambda_value = float(method.split()[1]) if method.startswith("lambda") else 1
    
    for seed in range(n_seeds):
        np.random.seed(seed)
        
        # Run reinforce and get metrics dictionary
        metrics = reinforce(method, lambda_param=lambda_value)

        # Store metrics in the all_metrics dictionary (no normalization)
        for key, value in metrics.items():
            all_metrics[key][i, seed] = value

# Compute averages across seeds for each method
#averaged_metrics = {metric: np.mean(all_metrics[metric], axis=1) for metric in all_metrics}
